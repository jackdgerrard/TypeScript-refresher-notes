...
// Define paths
const distDir: string = path.join(__dirname);
const markdownDir: string = path.join(path.resolve(__dirname, '..'), 'markdown');
...
async function generateHtmlFromMarkdown(): Promise<string> {
    try {
        const files: string[] = await fs.promises.readdir(markdownDir);
        const markdownFiles: string[] = files.filter(file => file.endsWith('.md') || file.endsWith('.markdown')).sort();

        let articlesHtml: string = '';
        for (const file of markdownFiles) {
            const filePath: string = path.join(markdownDir, file);
            const markdownContent: string = await fs.promises.readFile(filePath, 'utf8');
...
const server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
    if (req.url === '/') {
        const finalHtml: string = await generateHtmlFromMarkdown();
